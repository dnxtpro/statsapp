...
export class MatchLiveComponent implements OnInit, DoCheck {
  ...
  matchScore: MatchScore = {
    homeTeam: {
      points: this.latestMatchLocalscore,
      sets: this.latestMatchLocalSets,
    },
    awayTeam: {
      points: this.latestMatchVisitorscore,
      sets: this.latestMatchVisitorSets,
    },
    events: [],
  };
  registro: string = '';
  ...
  ngOnInit(): void {
    // Prefer matchId from URL if provided (route: /match-live/:matchId)
    const routeMatchId = Number(this.route.snapshot.paramMap.get('id')) || 0;
    if (routeMatchId > 0) {
      this.latestMatchId = routeMatchId;
      console.log('[MatchLive] Using matchId from URL:', this.latestMatchId);
    }
    ...
    setTimeout(() => {
      this.ultimoseventosss();
      if (
        this.matchScore.homeTeam.points >= 25 ||
        this.matchScore.awayTeam.points >= 25
      ) {
        if (
          this.matchScore.homeTeam.points - this.matchScore.awayTeam.points >=
          2
        ) {
        ...
        if (
          this.matchScore.awayTeam.points - this.matchScore.homeTeam.points >=
          2
        ) {
  ...
  loadLatestMatchDetails() {
    this.matchService.getLatestMatchDetails().subscribe(
      (latestMatchDetails) => {
        ...
        // Only set latestMatchId from backend if it wasn't provided via the URL
        if (!this.latestMatchId || this.latestMatchId === 0) {
      ...
      (error) => {
        console.error(
          'Error al obtener los detalles del partido más reciente:',
          error
        );
  ...
  handleFallo(falloType: string): void {
    ...
    if (
      this.selectedFaultType !== 'AciertoRival' &&
      this.selectedFaultType !== 'AciertoFintaRival' &&
      this.selectedFaultType !== 'AciertoZagueroRival'
    ){
      this.openPlayerModal('fallo', falloType);
    }
    else{
      const rivalPlayer = this.currentPlayers.find(player => player.name === 'Rival');
      if (rivalPlayer) {
        // Enviar el tipo de error y el jugador encontrado
        this.handleMatchEvent('fallo', rivalPlayer, falloType, rivalPlayer.name);
        console.log('RIVAL encontrado:', rivalPlayer.player_id, falloType, 'acierto');
      } else {
        console.log('Jugador RIVAL no encontrado');
      }
  ...
  handleAcierto(aciertoType: string): void {
    ...
    if (
      this.selectedFaultType !== 'ErrorSaqueRival' &&
      this.selectedFaultType !== 'ErrorAtaqueRival' &&
      this.selectedFaultType !== 'EXRival'
    ) {
      console.log("elrivalestamalito")
      this.openPlayerModal('acierto', aciertoType);
    } else {
      const rivalPlayer = this.currentPlayers.find(player => player.name === 'Rival');
      if (rivalPlayer) {
        // Enviar el tipo de error y el jugador encontrado
        this.handleMatchEvent('acierto', rivalPlayer, aciertoType, rivalPlayer.name);
        console.log('RIVAL encontrado:', rivalPlayer.player_id, aciertoType, 'acierto');
      } else {
        console.log('Jugador RIVAL no encontrado');
      }
  ...
  getEventTypeID(faultType: string, actionType: string): number | undefined {
    ...
    const aciertoso = this.aciertos.find(
      (aciertoso) => aciertoso.type === faultType && actionType === 'acierto'
    );
    ...
    if (aciertoso) {
      ...
      const falloso = this.fallos.find(
        (falloso) => falloso.type === faultType && actionType === 'fallo'
      );
  ...
  openPlayerModal(actionType: string, faultType: string): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        ...
        console.log('modnongo', actionType, player, faultType, playerName,this.previousTieneSaque);
  ...
  resetScores(): void {
    ...
    if (this.match1Id != this.latestMatchId) {
      ...
      if (this.matchScore.awayTeam.points == 0) {
        console.log(this.matchScore);
      }
    } else {
      console.log('si coinicden');
    }
